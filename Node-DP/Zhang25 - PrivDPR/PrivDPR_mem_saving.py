...
MY_CONFIG.graph_options.optimizer_options.global_jit_level = tf.compat.v1.OptimizerOptions.ON_1
...
def chunked_matmul(A, B, chunk_size=64):
    ...
    for i in range(0, A.shape[0], chunk_size):
        ...
        result_chunk = tf.matmul(A_chunk, B)
        result_chunks.append(result_chunk)
    return tf.concat(result_chunks, axis=0)
...
def tiled_matmul(A, B, tile_size=64):
    ...
    for i in range(0, n_rows, tile_size):
        row_chunk = A[i:i+tile_size]
        row_result = []
        for j in range(0, n_cols, tile_size):
            col_chunk = B[:, j:j+tile_size]
            prod = tf.matmul(row_chunk, col_chunk)
            row_result.append(prod)
        result.append(tf.concat(row_result, axis=1))
    return tf.concat(result, axis=0)

#def chunked_matmul(A, B_T, chunk_size=4096):
#    B_chunks = tf.split(B_T, num_or_size_splits=(B_T.shape[1] // chunk_size), axis=1)
#    return tf.concat([tf.matmul(A, chunk) for chunk in B_chunks], axis=1)
...
class DiGraSynModel:
    def __init__(self, graph, Layer_num, node_embed_init=None):
        ...
        with tf.compat.v1.variable_scope('graph_forward_pass'):
            if self.node_emd_init:
                self.node_embedding_matrix = tf.compat.v1.get_variable(name='node_embedding_mat',
            ...
            for l in range(Layer_num):
                ...
                if l == 0:
                    self.weights.append(tf.compat.v1.get_variable(name=weight_name, shape=[args.embedding_dim, args.hidden_layer_dim],
                                initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")), trainable=True))
                    self.biases.append(tf.compat.v1.get_variable(name=bias_name, shape=[args.hidden_layer_dim],
                                    initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")), trainable=True))
                elif l == Layer_num-1:
                    self.weights.append(tf.compat.v1.get_variable(name=weight_name, shape=[args.hidden_layer_dim, 1],
                                initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")), trainable=True))
                    self.biases.append(tf.compat.v1.get_variable(name=bias_name, shape=[1],
                                    initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")), trainable=True))
                else:
                    self.weights.append(tf.compat.v1.get_variable(name=weight_name, shape=[args.hidden_layer_dim, args.hidden_layer_dim],
                                initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")), trainable=True))
                    self.biases.append(tf.compat.v1.get_variable(name=bias_name, shape=[args.hidden_layer_dim],
                                    initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")), trainable=True))

            self.node_head_ids = tf.compat.v1.placeholder(tf.int64, shape=[None])

            self.node_tail_ids = tf.compat.v1.placeholder(tf.int64, shape=[None])

            self.sample_ids_set = tf.compat.v1.placeholder(tf.int64, shape=[None])

            self.node_head_outDeg = tf.compat.v1.placeholder(tf.float32, shape=[None])

            self.node_tail_inDeg = tf.compat.v1.placeholder(tf.float32, shape=[None])
            ...
            del self.node_embedding_matrix; gc.collect()
            #self.output_upped_w = tiled_matmul(self.node_embedding_matrix, tf.transpose(self.node_embedding_matrix))
            #with tf.device('/GPU:0'):
            self.output_discrete_batch_w = gumbel_softmax(self.output_upped_w, temperature=args.tau, hard=True)
            del self.output_upped_w; gc.collect()
            self.discrete_batch_w_index = tf.argmax(self.output_discrete_batch_w, axis=1)
            del self.output_discrete_batch_w; gc.collect()
            self.loss = tf.reduce_mean(self.AsyPreser_loss)
            self.optimizer = tf.compat.v1.train.AdamOptimizer(args.learning_rate)

            self.params = [v for v in tf.compat.v1.trainable_variables() if 'graph_forward_pass' in v.name]
    ...
    def Gau_Noise(self, tensor, sens_value):
        ...
        sigma = tf.sqrt(2 * tf.math.log(1.25 / (args.delta / total_iters))) / (float(args.epsilon) / total_iters)
        s = tensor.get_shape().as_list()  # get shape of the tensor
        Gau_noise = tf.random.normal(s, mean=0.0, stddev=sigma * sens_value)
        # t = tf.add(tensor, rt)
        return Gau_noise
    ...
    def random_walk(self, s, graph):
        ...
        while len(walk) < args.walk_len:
            ...
            if p not in graph or graph.degree(p) == 0:
                break
    ...
    def train(self, graph):
        ...
        with tf.compat.v1.Session(config=MY_CONFIG) as sess:
            sess.run(tf.compat.v1.global_variables_initializer())
            for each_epoch in range(args.n_epochs):
                idx_total = math.floor(len(self.node_list) / args.batch_size)
                for index in range(idx_total):
                    print(f'Epoch: {each_epoch}, Index: {index}/{idx_total}')
        ...
        return node_count_mat#, node_embeddings
...
def generate_SynGraphs(SynDigraName, num_of_edge, node_count_mat):#, node_embeddings):
    ...
    #print(type(syn_graph))
    #exit()
    with open(f"result/{SynDigraName}/nx_adj_eps{args.epsilon}_i{args.run_id}.pkl", "wb") as f:
        pickle.dump(syn_graph, f)
...
def spectral_norm(w, iteration=1):
    ...
    u = tf.Variable(initial_value=tf.random.truncated_normal(shape=[1, w_shape[-1]]), trainable=False)
...
def gumbel_softmax(logits, temperature, hard=False):
    ...
    y = chunked_gumbel_softmax_sample(logits, temperature, chunk_size=4096) #16384)
    if hard:
        y_max = tf.argmax(y, axis=1)
        y_hard = tf.one_hot(y_max, depth=tf.shape(y)[1], dtype=logits.dtype)
...
def gumbel_softmax_sample(logits, temperature):
    ...
    #tf.debugging.check_numerics(logits, "Logits had NaN or Inf")
    y = logits + sample_gumbel(tf.shape(logits))
    #with tf.device('/CPU:0'):
    #    return tf.nn.softmax(y / temperature)
    return tf.nn.softmax(y / temperature)

def chunked_gumbel_softmax_sample(x, temperature, chunk_size=2048):
    #tf.debugging.assert_all_finite(x, "Logits contain NaNs or Infs")
    n_rows = tf.shape(x)[0]
    result = tf.TensorArray(dtype=tf.float32, size=0, dynamic_size=True, infer_shape=False)
    i = tf.constant(0)
    def cond(i, result):
        return i < n_rows
    def body(i, result):
        end = tf.minimum(i + chunk_size, n_rows)
        chunk = x[i:end]
        chunk += sample_gumbel(tf.shape(chunk))
        softmax_chunk = tf.nn.softmax(chunk / temperature)
        result = result.write(i // chunk_size, softmax_chunk)
...
if __name__ == '__main__':
    Pre_name = 'Processed_Facebook.txt'
    ...
    # print(Layer_num)
    Alg_name = "_".join([dataset_name, f"eps{args.epsilon}", f"i{args.run_id}"]) + ".txt"
    ...
    del OriGraph, model; gc.collect()
    #generate_SynGraphs(Alg_name, num_of_edge, node_count_mat)#, node_embeddings)
    generate_SynGraphs(dataset_name, num_of_edge, node_count_mat)
