...
def run(nx_graph_path, feature_matrix_path, val_pos, val_neg, test_pos, test_neg, use_node2vec=False):
    ...
    # Use the entire private graph for training.
    edges_train = torch.tensor(all_edges, dtype=torch.long).t().to(device)
    # Use the "real" graph for the test/validation set.
    edges_val = val_pos.to(device)
    edges_test = test_pos.to(device)
    neg_val = val_neg.to(device)
    neg_test = test_neg.to(device)
    ...
    for epoch in tqdm(range(n_epochs), desc="GCN, Epoch"):
        ...
        labels = get_labels(edges_train, neg_train)#.to(device)
    ...
    if device != 'cpu':
        del model, optimizer, data, z, edges_train, edges_val, edges_test, neg_val, neg_test
        with torch.no_grad():
