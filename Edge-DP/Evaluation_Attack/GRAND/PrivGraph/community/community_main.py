...
def __comm_adjust_em(current_graph, status, weight_key, resolution, random_state, epsilon, divide):
    nb_pass_done = 0
    ...
    # If epsilon is None, we'll use non-private version
    if epsilon is None:
        while nb_pass_done < pass_max:
            ...
            for node in __randomize(current_graph.nodes(), random_state):
                com_node = status.node2com[node]
                # obtain all communities
                candi_communities = __allcom(node, current_graph, status, weight_key)
                
                remove_cost = - resolution * candi_communities.get(com_node, 0)
                # remove the node from the original community
                __remove(node, com_node, candi_communities.get(com_node, 0.), status)
                
                # Find the community with highest modularity gain
                best_incr = float('-inf')
                best_com = com_node
                
                for com, dnc in candi_communities.items():
                    incr = remove_cost + resolution * dnc
                    if incr > best_incr:
                        best_incr = incr
                        best_com = com
                
                # put the node into the best community
                __insert(node, best_com, candi_communities.get(best_com, 0.), status)
            
            new_mod = __modularity(status, resolution)
    else:
        # Original privacy-preserving version with Exponential Mechanism
