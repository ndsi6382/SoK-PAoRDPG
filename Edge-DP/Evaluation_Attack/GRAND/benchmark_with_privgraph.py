...
if __name__ == "__main__":
    ...
    argparser.add_argument("--dp_graph", type=str)
    ...
    for expe in range(number_of_experiments):
        with open(args.dp_graph, 'rb') as f:
            G_nx = pickle.load(f)
        G_dp = Graph.from_nx(G_nx)
        adj = G.adjacency_matrix()
        A = adj @ adj
        del adj, G_nx
        ...
        for Ga_prop, Ga in G.gradual_sample(sample_start, sample_stop, sample_increment):
            ...
            for expe_type in expe_types:
                ...
                #    proba_params, complete_graph, 0, end-start, f"logs/benchmark_with_privgraph/{name}.csv", Gstar, G, G_dp, epsilon)
                with open(f"./result/nx_adj_{name}_{Ga_prop}_{expe_type}.pkl", 'wb') as f:
                    pickle.dump(nx.Graph(Gstar.edges_no_repeat()), f)
