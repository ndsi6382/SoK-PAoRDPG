...
from src.utils import save_top_n, save_edge_num, sample_subgraph, save_top_n_sparse
import gc
#seed = 3

def generate_graph(adj_without_sigmoid, adj, threshold):
    if adj.shape[0] > 50000:
        #print("    More than 50k nodes.")
        generated_graph, gene_neck_value = save_top_n_sparse(adj_without_sigmoid, save_edge_num(adj), threshold=threshold)
    else:
...
def eval_generated_graph(args, epoch, model, adj, logger):
    ...
    params_graph = None
    with torch.no_grad():
        for i in tqdm(range(args.stat_generate_num), desc="Generating graph"):
            ...
            node_list = np.arange(adj.shape[0])
            adj_coo = adj.tocoo()
            ...
            adj_for_model = torch.sparse_coo_tensor(indices, values, torch.Size(adj.shape), dtype=torch.float32)
            mu, logvar, adj_without_sigmoid, _, _ = model.forward(node_list, adj_for_model, for_test=True)
...
def train(args, model_args, dataset, model, optimizer, logger):
    ...
    for epoch in tqdm(range(args.epochs), desc="Training Epoch"):
        optimizer.zero_grad()
        index = np.arange(dataset.adj.shape[0])
        # keep sparse
        adj = dataset.adj.tocoo()
        indices = torch.tensor(np.vstack((adj.row, adj.col)), dtype=torch.long) 
        values = torch.tensor(dataset.adj.tocoo().data, dtype=torch.float32)
        sub_adj = torch.sparse_coo_tensor(indices, values, torch.Size(adj.shape), dtype=torch.float32)
        del indices, values, adj; gc.collect(); #torch.cuda.empty_cache()
        #sub_adj = torch.FloatTensor(np.array(dataset.adj.todense()))
        mu, logvar_sub, reconst_adj, gan_pred, gan_label = model.forward(index, sub_adj, for_test=False)
        # Calculate loss
        loss = model.compiled_loss_function(reconst_adj, sub_adj, logvar_sub, pos_weight, gan_pred, gan_label)
        # Optimize
        assert torch.isnan(loss).sum() == 0 # if nan appears, change lr
        del index, mu, reconst_adj, sub_adj, logvar_sub, gan_pred, gan_label; gc.collect(); #torch.cuda.empty_cache()
        loss.backward()
        ...
        if stop_signal:
            print("Ran out of DP budget at Epoch {}.".format(epoch))
            break
