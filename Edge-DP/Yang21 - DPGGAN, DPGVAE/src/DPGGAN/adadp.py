...
class ADADP(Optimizer):
    ...
    def step2(self, tol=1.0):
        for group in self.param_groups:
            err_e = 0.0
            device = self.p1[0].device
            ones = torch.one_like(self.p1[0], device=device)
            
            for ijk,p in enumerate(group['params']):
                p.data.add_(-0.5*self.lrs, p.grad.data)
                
                denom = torch.max(ones, self.p1[ijk].pow(2))
                diff = (self.p1[ijk] - p.data).pow(2) / denom
                err_e += diff.norm(1)
        ...
        del self.p1; gc.collect(); #torch.cuda.empty_cache()
