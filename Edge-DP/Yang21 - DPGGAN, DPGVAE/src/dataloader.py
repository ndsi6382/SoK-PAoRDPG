...
def generate_feature(adj, n_eigenvector):
    ...
    if node_num >= 50000:
        try:
            _, adj_features = sp.linalg.eigsh(adj_normalized, k=n_eigenvector, which="LA", tol=1e-12)
        except sp.linalg.ArpackNoConvergence as e:
            adj_features = e.eigenvectors
            print(f"Found {adj_features.shape[1]} eigenvectors.")
    else:
        _, adj_features = sp.linalg.eigsh(adj_normalized, k=n_eigenvector, which='LA')
    actual_feature_dim = adj_features.shape[1]
...
class Single_Graph_Dataset:
    ...
    def load_data(self):
        if self.dataset_str == 'karate':
            ...
            with open(root_path + '/data/orig/' + self.dataset_str, 'rb') as tf:
                self.graph_adj = pickle.load(tf)
                adj = nx.to_scipy_sparse_matrix(self.graph_adj)
