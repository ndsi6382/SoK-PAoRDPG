...
from src.dataloader import Multi_Graph_Dataset, Single_Graph_Dataset
...
def arg_parser():
    ...
    parser.add_argument('--epsilon', type=float, default=10.0, help='Privacy budget, epsilon.')
    
    parser.add_argument('--seed', type=int, default=43, help='Random seed.')
    parser.add_argument('--threads', type=int, default=os.cpu_count(), help='Thread number.')
    parser.add_argument('--txt_log', type=bool, default=False, help='whether save the txt_log.')
...
def set_env(args):
    ...
    model_args = load_config(args.model_name, args.dataset_str, args.optimizer, epsilon=args.epsilon)
...
def load_data(args, dataset_str):
    ...
    dataset = Single_Graph_Dataset(dataset_str, args.n_eigenvector, )
...
def save_data(args, data_list):
    if 'DP' in args.model_name:
        ...
        save_path = root_path + '/data/generated/{}_{}_eps{}.pkl'.format(args.model_name, os.path.basename(args.dataset_str), args.model_args.eps_requirement)
    else:
        #save_path = root_path + '/data/generated/{}_{}.pkl'.format(args.model_name, args.dataset_str)
        save_path = root_path + '/data/generated/{}_{}.pkl'.format(args.model_name, os.path.basename(args.dataset_str))
    
    save_path = root_path + f"/{args.model_name}_result/{args.dataset_str.split('/')[-2]}/nx_adj_eps{args.epsilon}_i{args.seed}.pkl" # I know this is bad.
