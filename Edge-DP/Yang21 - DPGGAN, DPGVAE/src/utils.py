...
def save_top_n_sparse(adj, n, threshold=None):
    adj = adj.copy()
    np.fill_diagonal(adj, -np.inf)  # ignore diagonal
    triu_indices = np.triu_indices_from(adj, k=1)
    vals = adj[triu_indices]
    # Get top-n indices without full sort
    top_n_idx = np.argpartition(vals, -n)[-n:]
    row = triu_indices[0][top_n_idx]
    col = triu_indices[1][top_n_idx]
    if threshold is None:
        from scipy.sparse import coo_matrix
        top = coo_matrix((np.ones_like(row), (row, col)), shape=adj.shape)
        symmetric_top = top + top.T
        return symmetric_top.toarray(), 0
    else:
        neck_value = adj[row[0], col[0]]
        adj[adj >= threshold] = 1
        adj[adj < threshold] = 0
        return adj, neck_value


def save_edge_num(graph, undirected=True):
    graph = graph - sp.dia_matrix((graph.diagonal()[np.newaxis, :], [0]), shape=graph.shape)
    graph.eliminate_zeros()
    assert np.allclose(graph.diagonal(), 0)
    #original_graph = nx.from_scipy_sparse_matrix(graph)
    #n = original_graph.number_of_edges()
    return graph.nnz // 2 if undirected else graph.nnz
